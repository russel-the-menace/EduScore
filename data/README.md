...
`output/国内头部大学汇总.csv` 共 590 行，字段为：
...
`output/国内非头部大学汇总.csv` 共 2,381 行，字段为：
...
`output/国外头部大学汇总.csv` 共 2,250 行，字段为：
...
该表只包含带 US News 名次的院校，按名次升序，同名次保持 US News 页面顺序，并补充 US News 原始记录的城市所在地及 QS 名次。所在地可用于区分排名名称未明确标注的校区，例如中国石油大学的青岛记录。中文名先清空后重新匹配，优先级为港澳台人工对照、软科中国院校库、中留服英文名匹配、DeepSeek 补充。首轮英文匹配后，再为未进入头部表的 QS 院校生成中文名，并按中文名、国家及高置信英文别名回填 QS。

### 国外非头部大学汇总

`output/国外非头部大学汇总.csv` 共 6,171 行，字段为：
